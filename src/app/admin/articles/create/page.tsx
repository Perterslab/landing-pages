...
export default function CreateArticlePage() {
  ...
  // 插入网络图片逻辑
  const insertImage = () => {
    const url = prompt("请输入网络图片的URL链接 (例如 https://...)");
    if (url) {
    ...
    <div style={{ display: "flex", minHeight: "100vh", color: "#f8fafc" }}>
      ...
      <main style={{ flex: 1, padding: "60px" }}>
        <div style={{ maxWidth: "800px", background: "#1e293b", padding: "40px", borderRadius: "16px", border: "1px solid #334155" }}>
          <FormGroup label="文章标题"><input placeholder="输入一个吸引人的标题..." style={inputStyle} value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} /></FormGroup>
          <FormGroup label="文章摘要 (选填)"><textarea placeholder="简短总结核心内容..." style={{ ...inputStyle, height: "100px" }} value={formData.summary} onChange={e => setFormData({...formData, summary: e.target.value})} /></FormGroup>
          
          <FormGroup label="正文内容 (富文本编辑)">
            <div style={{ border: "1px solid #334155", borderRadius: "8px", overflow: "hidden" }}>
              <div style={{ padding: "10px", background: "#0f172a", borderBottom: "1px solid #334155", display: "flex", gap: "5px", flexWrap: "wrap" }}>
                ...
                {/* 新增插入图片按钮 */}
                <button onClick={insertImage} style={{ ...btnStyle, backgroundColor: "#10b981" }}>🖼️ 插入网络图片</button>
              </div>
