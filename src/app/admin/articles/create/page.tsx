...
export default function CreateArticlePage() {
  ...
  const handleSave = async () => {
    ...
    if (!error) { router.push("/admin/articles"); } else { alert("发布失败：" + error.message); }
    setLoading(false);
  };
    ...
    <div style={{ display: "flex", minHeight: "100vh", color: "#f8fafc" }}>
      ...
      <main style={{ flex: 1, padding: "60px" }}>
        <div style={{ maxWidth: "800px", background: "#1e293b", padding: "40px", borderRadius: "16px", border: "1px solid #334155" }}>
          ...
          <FormGroup label="文章摘要 (选填)"><textarea placeholder="简短总结这篇文章的核心内容..." style={{ ...inputStyle, height: "100px" }} value={formData.summary} onChange={e => setFormData({...formData, summary: e.target.value})} /></FormGroup>
