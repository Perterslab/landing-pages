...
export default function CreateProductPage() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", color: "#f8fafc", backgroundColor: "#0f172a" }}>
      <style>{`.editor:empty:before { content: attr(data-placeholder); color: #64748b; cursor: text; } .editor img { max-width: 100%; border-radius: 8px; margin: 10px 0; border: 1px solid #334155; }`}</style>
      ...
      <aside style={{ width: "320px", borderRight: "1px solid #1e293b", padding: "40px 24px", flexShrink: 0 }}>
        ...
        <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px", display: "flex", flexDirection: "column", gap: "15px" }}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", color: "#fbbf24" }}>
            <input type="checkbox" checked={formData.is_featured} onChange={e => setFormData({...formData, is_featured: e.target.checked})} style={{ width: "18px", height: "18px" }}/> ⭐️ 设为主推项目
          </label>
        </div>
      ...
      <main style={{ flex: 1, padding: "40px 60px", overflowY: "auto" }}>
        <div style={{ maxWidth: "800px", background: "#1e293b", padding: "40px", borderRadius: "16px", border: "1px solid #334155" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
            <FormGroup label="产品名称"><input placeholder="如: LexGuard AI" style={inputStyle} value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></FormGroup>
            <FormGroup label="URL 别名 (Slug)"><input placeholder="lexguard-ai" style={inputStyle} value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value.toLowerCase().replace(/\s+/g, '-')})} /></FormGroup>
          </div>
          
          <FormGroup label="简短描述 (Summary)"><textarea placeholder="摘要说明..." style={{ ...inputStyle, height: "80px" }} value={formData.summary} onChange={e => setFormData({...formData, summary: e.target.value})} /></FormGroup>
          ...
          <div style={{ padding: "20px", background: "#0f172a", borderRadius: "8px", border: "1px solid #334155", marginBottom: "24px" }}>
            ...
            <FormGroup label="结账链接 (如 Gumroad)"><input placeholder="https://..." style={inputStyle} value={formData.checkout_url} onChange={e => setFormData({...formData, checkout_url: e.target.value})} /></FormGroup>
            <FormGroup label="外部详情页链接"><input placeholder="https://..." style={inputStyle} value={formData.external_url} onChange={e => setFormData({...formData, external_url: e.target.value})} /></FormGroup>
          </div>
          ...
          <FormGroup label="图文详情 (Description)">
            <div style={{ border: "1px solid #334155", borderRadius: "8px", overflow: "hidden" }}>
              ...
              <div ref={editorRef} contentEditable className="editor" data-placeholder="详细介绍内容..." style={{ minHeight: "300px", padding: "20px", background: "#1e293b", outline: "none", lineHeight: "1.8" }} />
            </div>
          </FormGroup>
        </div>
      </main>
