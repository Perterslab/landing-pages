...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [formData, setFormData] = useState({ 
    ...
    external_url: "", checkout_url: "", is_published: false 
  });

  useEffect(() => {
    const fetchProduct = async () => {
      const { data } = await supabaseBrowserClient.from("products").select("*").eq("id", productId).single();
      if (data) {
        setFormData(data);
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabaseBrowserClient.from("products").update({ 
      ...formData, description: htmlContent 
    }).eq("id", productId);
  ...
  if (fetching) return <div style={{ padding: "40px", color: "#94a3b8", backgroundColor: "#0f172a", minHeight: "100vh" }}>读取数据中...</div>;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", color: "#f8fafc", backgroundColor: "#0f172a" }}>
      ...
      <aside style={{ width: "320px", borderRight: "1px solid #1e293b", padding: "40px 24px", flexShrink: 0 }}>
        <h2 style={{ color: "#3b82f6", marginBottom: "30px" }}>编辑产品信息</h2>
        ...
        <div style={{ background: "#1e293b", padding: "20px", borderRadius: "12px" }}>
          <label style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
            <input type="checkbox" checked={formData.is_published} onChange={e => setFormData({...formData, is_published: e.target.checked})} style={{ width: "18px", height: "18px" }}/> 上架状态
          </label>
        </div>
      ...
      <main style={{ flex: 1, padding: "40px 60px", overflowY: "auto" }}>
        <div style={{ maxWidth: "800px", background: "#1e293b", padding: "40px", borderRadius: "16px", border: "1px solid #334155" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
            ...
            <FormGroup label="URL 别名 (Slug)"><input style={inputStyle} value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value.toLowerCase().replace(/\s+/g, '-')})} /></FormGroup>
          </div>
          
          <FormGroup label="简短描述 (Summary)"><textarea style={{ ...inputStyle, height: "80px" }} value={formData.summary} onChange={e => setFormData({...formData, summary: e.target.value})} /></FormGroup>
          ...
          <div style={{ padding: "20px", background: "#0f172a", borderRadius: "8px", border: "1px solid #334155", marginBottom: "24px" }}>
            ...
            <FormGroup label="直接购买/结账链接 (如 Gumroad)">
          ...
          <FormGroup label="图文详情 (Description)">
            <div style={{ border: "1px solid #334155", borderRadius: "8px", overflow: "hidden" }}>
              ...
              <div ref={editorRef} contentEditable className="editor" style={{ minHeight: "300px", padding: "20px", background: "#1e293b", outline: "none", lineHeight: "1.8", fontSize: "1rem" }} />
            </div>
          </FormGroup>
        </div>
      </main>
